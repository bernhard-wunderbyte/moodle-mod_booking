* New feature: E-mail templates for the three e-mail modals in the Bookings Tracker (send custom message, message to teacher(s), message to responsible contact(s)). A "Save template" button stores subject and message as a template right away - without sending a message (new table booking_emailtemplates; optionid set = option template, otherwise cmid set = instance template, both 0 = global). Tiered permissions for the template scope: templates for the booking option need no extra capability (whoever can open the option's tracker may save them), instance-wide templates require the new capability mod/booking:customemailtemplates (default: editingteacher, manager) in the module context, global templates require it in the system context - the scope select only offers what the user may use, and the server clamps anything else to the option scope. Everyone using the modals can load the applicable templates (option, then instance, then global ones) into subject and message via a picker - no extra JS, loading and saving run through no-submit reloads of the dynamic form. Attachments are not part of templates.
